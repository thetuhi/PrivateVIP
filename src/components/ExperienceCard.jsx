 ...
 * Catalog card.
 *
 * The whole card is one link, an overlay pseudo-element stretched across it,
 * rather than several competing hit areas. One tab stop, the title as the
 * accessible name, and a thumb landing anywhere on the card works.
 *
      ...
      <TiltCard max={3} lift={12} className="flex h-full flex-col">
        ...
        <div className="flex flex-1 flex-col pt-5" data-depth="0.4">
          <h3 className="font-display text-2xl leading-tight text-bone transition-colors duration-base ease-enter group-hover:text-brass-300">
            <Link to={`/experiences/${experience.slug}`} className="after:absolute after:inset-0 after:content-['']">
              {title}
            </Link>
          </h3>
